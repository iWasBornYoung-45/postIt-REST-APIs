...
async def get_posts(db: Annotated[Session, Depends(get_db)], id: UUID | None  = None, limit: int | None = 5, skip: int | None = 0, search: str | None = ""):
    if not id:
        query = db.query(models.Posts, func.count(models.Upvotes.post_id).label("upvote_count")).join(models.Upvotes, models.Posts.id == models.Upvotes.post_id, isouter=True).filter(models.Posts.published == True, models.Posts.title.contains(search)).group_by(models.Posts.id).limit(limit).offset(skip)
    else:
        query = db.query(models.Posts, func.count(models.Upvotes.post_id).label("upvote_count")).join(models.Upvotes, models.Posts.id == models.Upvotes.post_id, isouter=True).filter(models.Posts.published == True, models.Posts.id == id).group_by(models.Posts.id)
    res = query.all()
